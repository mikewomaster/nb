#ifndef MBUS_PROTOCOL_H
#define MBUS_PROTOCOL_H
...
typedef struct _mbus_frame {
    ...
    struct _mbus_frame *next; // pointer to next mbus_frame for multi-telegram replies
...
//
//
#define MBUS_MAX_PRIMARY_SLAVES 1//256
...
//
mbus_data_record *mbus_data_record_new(void);
...
mbus_frame *mbus_frame_alloc(void);
void mbus_frame_alloc_free(mbus_frame *data);
...
int mbus_decode_manufacturer(unsigned char byte1, unsigned char byte2, char *buf, int buf_len);
...
int mbus_data_fixed_unit(int medium_unit_byte, char *buf, int buf_len);
int mbus_data_variable_medium_lookup(unsigned char medium, char *buf, int buf_len);
int mbus_unit_prefix(int exp, char *buf, int buf_len);

const char *mbus_data_error_lookup(int error);

int mbus_vib_unit_lookup(mbus_value_information_block *vib, char *buf, int buf_len);
int mbus_vif_unit_lookup(unsigned char vif, char *buf, int buf_len);
...
extern mbus_frame g_mbus_frame;
extern mbus_frame g_mbus_reply_frame;
extern mbus_frame_data g_mbus_frame_data;

#endif // MBUS_PROTOCOL_H

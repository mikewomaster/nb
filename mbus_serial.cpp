...
void MainWindow::serialReadReady2()
{
    ...
    if (reply->error() == QModbusDevice::NoError) {
            ...
            ui->s_baudCombo_2->setCurrentIndex(baudRate);

            quint32 dataBit = unit.value(1);
            ui->s_dataBitsCombo_2->setCurrentText(QString::number(dataBit));

            quint32 stopBit = unit.value(2);
            ui->s_stopBitsCombo_2->setCurrentText(QString::number(stopBit));

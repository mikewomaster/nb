...
/*  Primary VIFs (main table), range 0x00 - 0xFF */
    ...
    { 0x58, 1.0e-3, "deg C", "Flow temperature" },
    { 0x59, 1.0e-2, "deg C", "Flow temperature" },
    { 0x5A, 1.0e-1, "deg C", "Flow temperature" },
    { 0x5B, 1.0e0,  "deg C", "Flow temperature" },

    /* E101 11nn Return Temperature °C (0.001°C to 1°C) */
    { 0x5C, 1.0e-3, "deg C", "Return temperature" },
    { 0x5D, 1.0e-2, "deg C", "Return temperature" },
    { 0x5E, 1.0e-1, "deg C", "Return temperature" },
    { 0x5F, 1.0e0,  "deg C", "Return temperature" },
    ...
    { 0x64, 1.0e-3, "deg C", "External temperature" },
    { 0x65, 1.0e-2, "deg C", "External temperature" },
    { 0x66, 1.0e-1, "deg C", "External temperature" },
    { 0x67, 1.0e0,  "deg C", "External temperature" },
...
/* Alternate VIFE-Code Extension table (following VIF=0FBh for primary VIF)
   See 8.4.4 b, only some of them are here. Using range 0x200 - 0x2FF */
    ...
    { 0x238, 1.0e-3, "deg F", "Flow temperature" },
    { 0x239, 1.0e-2, "deg F", "Flow temperature" },
    { 0x23A, 1.0e-1, "deg F", "Flow temperature" },
    { 0x23B, 1.0e0,  "deg F", "Flow temperature" },

    /* E101 11nn Return Temperature 10(nn-3) °F 0.001°F to 1°F */
    { 0x23C, 1.0e-3, "deg F", "Return temperature" },
    { 0x23D, 1.0e-2, "deg F", "Return temperature" },
    { 0x23E, 1.0e-1, "deg F", "Return temperature" },
    { 0x23F, 1.0e0,  "deg F", "Return temperature" },

    /* E110 00nn Temperature Difference 10(nn-3) °F 0.001°F to 1°F */
    { 0x240, 1.0e-3, "deg F", "Temperature difference" },
    { 0x241, 1.0e-2, "deg F", "Temperature difference" },
    { 0x242, 1.0e-1, "deg F", "Temperature difference" },
    { 0x243, 1.0e0,  "deg F", "Temperature difference" },

    /* E110 01nn External Temperature 10(nn-3) °F 0.001°F to 1°F */
    { 0x244, 1.0e-3, "deg F", "External temperature" },
    { 0x245, 1.0e-2, "deg F", "External temperature" },
    { 0x246, 1.0e-1, "deg F", "External temperature" },
    { 0x247, 1.0e0,  "deg F", "External temperature" },
    ...
    { 0x250, 1.0e-3, "deg F", "Cold / Warm Temperature Limit" },
    { 0x251, 1.0e-2, "deg F", "Cold / Warm Temperature Limit" },
    { 0x252, 1.0e-1, "deg F", "Cold / Warm Temperature Limit" },
    { 0x253, 1.0e0,  "deg F", "Cold / Warm Temperature Limit" },

    /* E111 01nn Cold / Warm Temperature Limit 10(nn-3) °C 0.001°C to 1°C */
    { 0x254, 1.0e-3, "deg C", "Cold / Warm Temperature Limit" },
    { 0x255, 1.0e-2, "deg C", "Cold / Warm Temperature Limit" },
    { 0x256, 1.0e-1, "deg C", "Cold / Warm Temperature Limit" },
    { 0x257, 1.0e0,  "deg C", "Cold / Warm Temperature Limit" },
...
const mbus_variable_vif fixed_table[] = {
    /* 00, 01 left out */
...
mbus_vif_unit_normalize(int vif, double value, char **unit_out, double *value_out, char **quantity_out)
{
    ...
    for(i=0; vif_table[i].vif < 0xfff; ++i)
    {
        ...
        {
            *unit_out = (char *)(vif_table[i].unit);//strdup(vif_table[i].unit);
            *value_out = value * vif_table[i].exponent;
            *quantity_out = (char *)(vif_table[i].quantity);//strdup(vif_table[i].quantity);
            return 0;
        }
    }
